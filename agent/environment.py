...
class ReversiEnv(gym.Env):
    def __init__(self, board: Optional[reversi.Board] = None):
        ...
        self._action_to_move = lambda action: (action // 8, action % 8) # Convert index to (row, col)
    ...
    def _get_obs(self):
        ...
        return np.array([
            [cell_state_to_value(self.board[(row, col)], self.board.player_to_move) for col in range(8)]
            for row in range(8)
        ], dtype=np.int8)
    ...
    def step(self, action):
        ...
        piece_diff = reversi.piece_count(self.board, self.board.player_to_move) - reversi.piece_count(self.board, self.board.player_to_move.opponent())
        if terminated:
            friendly_count = reversi.piece_count(self.board, self.board.player_to_move)
            opponent_count = reversi.piece_count(self.board, self.board.player_to_move.opponent())
            if not is_valid:
                reward = -1  # Invalid move penalty (should be filtered by masking)
            ...
            reward = piece_diff / 6400 - 0.01  # Normalize reward by total number of squares, with a small step penalty to encourage faster wins
